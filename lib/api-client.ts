...
class FireflyApiClient {
  ...
  private handleError(error: AxiosError): Error {
    if (error.response) {
      ...
      switch (status) {
        case 401:
          return new Error('Invalid credentials. Please check your Personal Access Token.');
        case 403:
          return new Error('Access forbidden. You do not have permission to access this resource.');
        case 404:
      ...
      return new Error('Cannot connect to Firefly III. Please check your instance URL and network connection.');
    } else {
  ...
  private ensureInitialized() {
    if (!this.axiosInstance) {
      throw new Error('API client not initialized. Please set credentials first.');
    }
  ...
  async getTransactions(
    ...
    const api = this.ensureInitialized();
    const response = await api.get<FireflyApiResponse<Transaction[]>>('transactions', {
  ...
  async getTransaction(id: string): Promise<FireflyApiResponse<Transaction>> {
    const api = this.ensureInitialized();
    const response = await api.get<FireflyApiResponse<Transaction>>(`transactions/${id}`);
    return response.data;
  }

  async createTransaction(data: CreateTransactionData): Promise<FireflyApiResponse<Transaction>> {
    const api = this.ensureInitialized();
    const response = await api.post<FireflyApiResponse<Transaction>>('transactions', data);
    return response.data;
  }

  async updateTransaction(id: string, data: CreateTransactionData): Promise<FireflyApiResponse<Transaction>> {
    const api = this.ensureInitialized();
    const response = await api.put<FireflyApiResponse<Transaction>>(`transactions/${id}`, data);
    return response.data;
  }
  ...
  async createBudget(data: CreateBudgetData): Promise<FireflyApiResponse<Budget>> {
    const api = this.ensureInitialized();
    const response = await api.post<FireflyApiResponse<Budget>>('budgets', data);
    return response.data;
  }

  async updateBudget(id: string, data: Partial<CreateBudgetData>): Promise<FireflyApiResponse<Budget>> {
    const api = this.ensureInitialized();
    const response = await api.put<FireflyApiResponse<Budget>>(`budgets/${id}`, data);
    return response.data;
  }
  ...
  async getBudgetLimits(id: string): Promise<FireflyApiResponse<BudgetLimit[]>> {
    const api = this.ensureInitialized();
    const response = await api.get<FireflyApiResponse<BudgetLimit[]>>(`budgets/${id}/limits`);
    return response.data;
  }

  // Piggy Banks
  async getPiggyBanks(page: number = 1): Promise<FireflyApiResponse<PiggyBank[]>> {
    const api = this.ensureInitialized();
    const response = await api.get<FireflyApiResponse<PiggyBank[]>>('piggy-banks', {
      params: { page },
    });
    return response.data;
  }
  ...
  async getPiggyBank(id: string): Promise<FireflyApiResponse<PiggyBank>> {
    const api = this.ensureInitialized();
    const response = await api.get<FireflyApiResponse<PiggyBank>>(`piggy-banks/${id}`);
    return response.data;
  }

  // Recurring Transactions (Subscriptions)
  async getRecurringTransactions(page: number = 1): Promise<FireflyApiResponse<RecurringTransaction[]>> {
    const api = this.ensureInitialized();
    const response = await api.get<FireflyApiResponse<RecurringTransaction[]>>('recurring', {
      params: { page },
    });
    return response.data;
  }
