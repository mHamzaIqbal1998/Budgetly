...
export default function EditBudgetScreen() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setIsLoading(true);
      try {
        const [budgetResponse, currenciesList] = await Promise.all([
          apiClient.getBudget(id),
          apiClient.getUserCurrencies().catch(() => [] as UserCurrenciesList[]),
        ]);

        const b = (budgetResponse as FireflyApiResponse<Budget>).data;
        setBudget(b);
        ...
        // Auto-budget
        const abt = attrs.auto_budget_type || "none";
        if (abt === "none" || abt === "null" || !abt) {
          setAutoBudgetType("none");
        } else {
          setAutoBudgetType(abt);
        }
        setAutoBudgetCurrencyCode(attrs.currency_code || "");
        setAutoBudgetAmount(attrs.auto_budget_amount || "");
        setAutoBudgetPeriod(attrs.auto_budget_period || "monthly");
    ...
    try {
      ...
      if (apiAutoBudgetType !== "none") {
        ...
        body.auto_budget_period =
          (autoBudgetPeriod as UpdateBudgetData["auto_budget_period"]) || null;
      ...
      await apiClient.updateBudget(id, body);
      ...
      queryClient.removeQueries({ queryKey: ["budget-detail", id] });
      queryClient.removeQueries({
        queryKey: ["budget-limits-detail", id],
      });
  ...
  if (isLoading) {
    return (
      ...
      >
        ...
        <Text style={{ marginTop: 16 }}>Loading budget...</Text>
      </View>
    );
  }
