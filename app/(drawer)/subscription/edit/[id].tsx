...
export default function EditSubscriptionScreen() {
  ...
  // ---------------------------------------------------------------------------
  // Fetch existing data + currencies
  // ---------------------------------------------------------------------------
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const [billRes, currenciesList] = await Promise.all([
          apiClient.getBill(id!),
          apiClient.getUserCurrencies().catch(() => [] as UserCurrenciesList[]),
        ]);
        setUserCurrencies(Array.isArray(currenciesList) ? currenciesList : []);

        const attrs = billRes.data?.attributes;
        if (attrs) {
          setName(attrs.name || "");
          setAmountMin(attrs.amount_min || "");
          setAmountMax(attrs.amount_max || "");
          setCurrencyCode(attrs.currency_code || "");
          setDate(parseDate(attrs.date) || new Date());
          setEndDate(parseDate(attrs.end_date));
          setExtensionDate(parseDate(attrs.extension_date));
          setRepeatFreq(attrs.repeat_freq || "monthly");
          setSkip(String(attrs.skip ?? 0));
