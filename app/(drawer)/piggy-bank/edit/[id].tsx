...
export default function EditPiggyBankScreen() {
  ...
  // Fetch piggy bank data and accounts
  useEffect(() => {
    let mounted = true;
    ...
    async function fetchData() {
      ...
      try {
        ...
        // Set selected account from first linked account
        const firstAccount = data.attributes.accounts[0];
        if (firstAccount) {
  ...
  }, [navigation, name, isLoading, router, theme.colors.onSurface]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Build accounts array - update with selected account while preserving current amount
      const currentAmount =
        piggyBank.attributes.accounts[0]?.current_amount || "0";
      ...
      queryClient.removeQueries({ queryKey: ["piggy-bank-detail", id] });
      ...
      >
        ...
        <GlassCard variant="elevated" style={styles.formCard}>
          <Card.Content style={styles.formContent}>
            ...
            {/* Active Toggle as theres no active in update endpoint */}
